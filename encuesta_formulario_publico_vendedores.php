...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $e = encuestas_guardar_vendedor($pdo, null);
    if ($e === null) {
        $mensajeExito = true;
    } else {
        $error = $e;
...
<head>
    ...
    <title>Encuesta — Formulario público (vendedores)</title>
    ...
    <style>
        ...
        .enc-head { background: linear-gradient(135deg, #0d6efd 0%, #0a58ca 100%); color: #fff; border-radius: 15px; padding: 1.5rem; margin-bottom: 1.25rem; }
        .preg-block { background: #fff; border-radius: 12px; padding: 1.25rem; margin-bottom: 1rem; box-shadow: 0 2px 12px rgba(0,0,0,0.06); }
        .enc-puntuacion .btn-check:checked + .btn { background: linear-gradient(180deg, #ffecb3, #ffc107); border-color: #ffc107; color: #1a1a1a; font-weight: 600; }
...
<body>
    ...
    <div class="container enc-wrap py-4">
        <div class="enc-head">
            <h1 class="h4 mb-1"><i class="fas fa-star me-2"></i>Encuesta: formulario público</h1>
            <p class="mb-0 small opacity-90">Dirigida a vendedores. Evalue su experiencia al compartir el enlace con clientes. Cada pregunta: 1 a 5 estrellas.</p>
        </div>
        ...
        <?php else: ?>
            ...
            <form method="post" class="encuesta" action="" autocomplete="on">
                ...
                <?php foreach ($ENCUESTA_VENDEDOR_PREGUNTAS as $num => $texto): ?>
                <div class="preg-block">
                    ...
                    <div class="btn-group enc-puntuacion" role="group" aria-label="Puntuación de la pregunta <?php echo (int) $num; ?>">
                        ...
                        for ($e = 1; $e <= 5; $e++) {
                            ...
                            echo '<label class="btn btn-outline-warning" for="' . htmlspecialchars($id) . '">';
                            for ($i = 0; $i < $e; $i++) {
                ...
                <div class="preg-block">
                    ...
                    <textarea class="form-control" name="recomendaciones" id="recomendaciones" rows="4" maxlength="10000" placeholder="Sugerencias, problemas frecuentes, ideas de mejora..."><?php echo htmlspecialchars((string) ($_POST['recomendaciones'] ?? '')); ?></textarea>
                </div>
